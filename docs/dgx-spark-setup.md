...
- **実機検証による画像生成 (1.6B) の実測性能**:
  ...
  * **詳細な計測条件**: 精度 `bfloat16` (`bf16`), 20 steps, `flow_dpm-solver` サンプラー、`xformers` memory-efficient attention バックエンド。ウォームアップ完了後の3回計測における中央値。
  * **初期オーバーヘッド**: モデルロード（Hugging Faceキャッシュから）に約8.7秒、初回推論（JITコンパイル等含む初期化）に約15秒かかりますが、2回目以降の連続実行時には6.46秒で極めて安定して動作します。
- **パフォーマンスと制限（SANA-WMの想定リスク）**: 一方で、SANA-WM の特徴である **60秒クラスの長尺 720p 動画生成** を実行する際には、アテンションの計算量がシーケンス長の増加に伴って二乗で爆発するため、flash-attn によるメモリ削減と高速化の恩恵が得られないと、128GBのUnified Memory環境下であってもOOM（OutOfMemory）または処理ハングを引き起こす可能性が高い想定（未検証、flash-attnなしでは厳しい見込み）です。そのため、長尺動画系よりも先に画像系（特にリアルタイム業務用途で重要な **SANA-Sprint 1step 1024px** など）のベンチマーク取得を優先することを推奨します。
